...
## המשימה
קרא את המסמך וחלץ את נתוני המשכנתא. המסמך יכול להיות:
1. **הצעת משכנתא חדשה** - טבלת "סל מוצע" עם מסלולים מוצעים
2. **דוח פרטי משכנתא קיימת** - פירוט הלוואות קיימות עם תאריכי התחלה

## זיהוי סוג המסמך
- אם יש "תאריך תחילת ההלוואה" / "תאריך העמדה" / "תאריך ביצוע" → זו משכנתא קיימת
- אם יש "יתרה לסילוק" / "יתרת קרן" → זו משכנתא קיימת
- אחרת → זו הצעה חדשה
...
7. **שיעור ריבית שנתית** - כאן יש מספר אפשרויות:
   - **נומינלית ליום X.XX%** או **ריבית נומינלית ליום** ← קח את זה! (הכי מעודכן)
   - **נומינלית X.XX%** ← קח את זה אם אין "ליום"
   - מתואמת X.XX% (להתעלם)
8. **ריבית משתנה: מנגנון קביעת הריבית** - ריבית בסיס ותוספת/הפחתה:
   - **P-X.XX% או P+X.XX%** ← זה פריים (prime)! P = Prime
   - **H+X.XX%** ← זה משתנה (variable)
...
| מה כתוב במסמך | type |
|---------------|------|
| פריים / פריים דיור / P-X% / P+X% / ריבית על בסיס פריים / עוגן פריים | prime |
| קבועה לא צמודה / ריבית קבועה לא צמודה / קבועה לא צמודה - דיור / קבועה לא צמודה בחלקים / קל"צ | fixed_unlinked |
| קבועה צמודה / צמודה - דיור / ריבית קבועה צמודה / צמודה למדד / קצ"מ | fixed_linked |
| משתנה לא צמודה / משתנה כל X שנים / 2+2 / H+X% / מק"מ / עוגן מק"מ / על בסיס אג"ח | variable_unlinked |
| משתנה צמודה / משתנה צמודה למדד / משתנה צמודה כל X שנים | variable_linked |

**חשוב מאוד:**
- P = Prime = פריים! אם רואים P-0.70% או P+1.5% זה תמיד prime
- H = משתנה (לא פריים)
...
1. **ריבית נומינלית** - תמיד קח את הריבית הנומינלית, לא המתואמת
2. **עדיפות לריבית מעודכנת** - אם יש "נומינלית ליום" או "ריבית נומינלית ליום" - זו הריבית הכי מעודכנת, קח אותה!
3. **אם כתוב "נומינלית X.XX%"** - קח את X.XX
4. **אם יש רק מספר אחד** בשורת הריבית - קח אותו
...
**חשוב - שדות:**
- **name**: העתק את שם המסלול המדויק כפי שמופיע במסמך
- **isExistingMortgage**: true אם זו משכנתא קיימת (דוח פרטי), false אם הצעה חדשה
- **originalPeriodMonths**: תקופת ההלוואה המקורית בחודשים
- **startDate**: תאריך תחילת ההלוואה (אם קיים במסמך) בפורמט YYYY-MM-DD
- **remainingMonths**: תקופה שנותרה בחודשים (אם כתוב במפורש במסמך)
- **amount**: עבור משכנתא קיימת - קח את "יתרה לסילוק" או "יתרת קרן". עבור הצעה חדשה - קח את סכום ההלוואה
